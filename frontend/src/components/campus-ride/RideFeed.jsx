...
const Avatar = ({ name, avatar, initials }) => (
  <div className="size-12 rounded-full overflow-hidden bg-gradient-to-br from-violet-500 to-purple-600 ring-2 ring-violet-500/20 flex items-center justify-center text-white font-semibold">
    {avatar ? <img src={avatar} alt={name} className="w-full h-full object-cover" /> : initials || name?.slice(0, 2)}
  </div>
);

function RideCard({ driver, pickup, dropoff, departureTime, availableSeats, totalSeats, price }) {
  return (
            ...
            {Array.from({ length: totalSeats }).map((_, i) => (
              <div
                ...
                className={`size-2 rounded-full transition-colors ${
                  i < availableSeats ? 'bg-gradient-to-br from-emerald-400 to-teal-500' : 'bg-slate-300'
                }`}
              />
            ))}
...
export default function RideFeed({ rides, title = 'Nearby Offers', subtitle = 'Optimized by proximity to you' }) {
  const fallbackRides = [
    {
      driver: {
        name: 'Sarah Chen',
  ...
  const list = rides && rides.length ? rides : fallbackRides;
